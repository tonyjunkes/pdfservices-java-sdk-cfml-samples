 ...
 * And to know more about fragments use-case in document generation and document templates, please see the http://www.adobe.com/go/dcdocgen_fragments_support
 *
 * Refer to README.md for instructions on how to run the samples.
 */
component displayname="Merge Document To DOCX With Fragments" {
    public MergeDocumentToDOCXWithFragments function init() {
        ...
        variables.java_Fragments = createObject("java", "com.adobe.pdfservices.operation.pdfops.options.documentmerge.Fragments");
    ...
    public void function run() {
        try {
            ...
            var jsonDataForMerge = variables.java_JSONObject.init(serializeJSON(initialData));

            // Create fragments from fragment JSON data
            var fragment1 = {
                "orderDetails": "<b>Quantity</b>:{{quantity}}, <b>Description</b>:{{description}}, <b>Amount</b>:{{amount}}"
            };
            var fragmentJSON1 = variables.java_JSONObject.init(serializeJSON(fragment1));

            var fragment2 = {
                "customerDetails": "{{customerName}}, Visits: {{customerVisits}}"
            };
            var fragmentJSON2 = variables.java_JSONObject.init(serializeJSON(fragment2));

            // Create Fragments List
            var fragments = variables.java_Fragments;

            // Add all fragments to the created fragments list
            fragments.addFragment(fragmentJSON1);
            fragments.addFragment(fragmentJSON2);
            ...
            var documentMergeOptions = variables.java_DocumentMergeOptions.init(jsonDataForMerge, variables.java_OutputFormat.DOCX, fragments);
            ...
            var documentTemplate = variables.java_FileRef.createFromLocalFile(
                application.resourcesPath & "/documentMergeFragmentsTemplate.docx"
            );
            ...
            result.saveAs(application.outputPath & "/documentMergeFragmentsOutput.docx");
        }
        catch(any e) {
