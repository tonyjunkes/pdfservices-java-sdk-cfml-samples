 ...
 */
component displayname="Extract Text Table Info With Figures Tables Renditions From PDF" {
    public ExtractTextTableInfoWithFiguresTablesRenditionsFromPDF function init() {
    ...
    public void function run() {
        try {
            ...
            var extractRenditionsList = variables.java_Arrays.asList([
                variables.java_ExtractRenditionsElementType.TABLES,
                variables.java_ExtractRenditionsElementType.FIGURES
            ]);
            var extractPDFOptions = variables.java_ExtractPDFOptions
                .extractPdfOptionsBuilder()
                .addElementsToExtract(extractElementList)
                .addElementsToExtractRenditions(extractRenditionsList)
                .build();
            ...
            result.saveAs(application.outputPath & "/ExtractTextTableInfoWithFiguresTablesRenditionsFromPDF.zip");
        }
        catch(any e) {
